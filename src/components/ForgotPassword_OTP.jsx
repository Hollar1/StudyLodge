...
//           type: "success",
//         });
//         // navigate("/Otp", { state: { email } }); (NO NEED TO NAVIGATE HERE)
//       } else {
//         Notify({
//           title: "Error",
//           message: response.data.Error || "An unexpected error occurred",
...
//               </div>
//               <button onClick={()=>{handleSendOTP(email)}} className={OtpStyles.resend}>Resend</button>
...
function ForgotPassword_OTP() {
  ...
  useEffect(() => {
    const banData = JSON.parse(localStorage.getItem("forgotPasswordOtpBan"));
    if (banData && banData.email === email) {
      ...
      if (currentTime < banData.banEndTime) {
        ...
        localStorage.removeItem("forgotPasswordOtpBan");
      }
    }
  }, [email]);
        ...
        <div className={OtpStyles.ramp}>
          ...
          <div className={OtpStyles.designA}>
            <div className={OtpStyles.container_div}>
              ...
              <div className={OtpStyles.otp_submit_btn_div}>
                <button disabled={signUpSpinner} type="submit">
                  Submit
                </button>
              </div>
            </div>
